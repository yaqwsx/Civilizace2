...
class ActionViewSet(viewsets.ViewSet):
    ...
    def typeInfo(outer_type: Type) -> Dict[str, Any]:
        if isinstance(outer_type, type):
            if issubclass(outer_type, Enum):
                return {
                    'type': 'Enum',
                    'values': {name: value.value for name, value in outer_type._member_map_.items()},
                }
            return {'type': outer_type.__name__}
        if typing.get_origin(outer_type) is not None:
            return {
                'type': typing.get_origin(outer_type).__name__,
                'subtypes': [ActionViewSet.typeInfo(subtype) for subtype in typing.get_args(outer_type)]
            }
    ...
    def serialize_action(action: GameAction):
        ...
        assert issubclass(
            action.action, TeamInteractionActionBase | NoInitActionBase)

        args = {name: ActionViewSet.fieldInfo(field)
    ...
    def list(self, request: Request):
        return Response([ActionViewSet.serialize_action(action) for action in GAME_ACTIONS.values()])
